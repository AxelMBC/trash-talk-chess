...
## Purpose
Define the engine-powered opponent: a "vs Computer" mode launched from the main menu with a selectable difficulty, where a client-side chess engine plays one side through the same animated move pipeline as human moves, with input lockout while the engine acts, deliberate-feeling reply timing, graceful failure handling, and a clean engine lifecycle across rematch and exit.
