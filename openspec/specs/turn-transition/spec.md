...
## Purpose
Define the board orientation change between turns in local pass-and-play: a single continuous, seamless in-plane rotation that reorients the board to face the player to move, with upright pieces, faded coordinate labels, input lockout during the transition, and a reduced-motion fallback.
